...
(function(global) {
  ...
  var Brain = function(num_agents, num_states, num_actions, opt) {
    ...
    this.state_windows = new Array(this.num_agents);
    this.action_windows = new Array(this.num_agents);
    this.reward_windows = new Array(this.num_agents);
    this.net_windows = new Array(this.num_agents);
    for (var i=0; i<num_agents; i++){
      this.state_windows[i] = new Array(this.window_size);
      this.action_windows[i] = new Array(this.window_size);
      this.reward_windows[i] = new Array(this.window_size);
      this.net_windows[i] = new Array(this.window_size);
    }
    ...
    if(typeof opt.layer_defs !== 'undefined') {
      ...
      // actions must check out. This is not very pretty Object Oriented programming but I can't see a way out of it :(
    ...
    this.forward_passes = new Array(this.num_agents);
    for (var i = 0; i < this.num_agents; i++){
      this.forward_passes[i] = 0.0;
    }

    this.epsilon = 1.0; // controls exploration exploitation tradeoff. Should be annealed over time
    //this.latest_reward = 0;
    //this.last_input_arrays = [];
    this.latest_rewards = new Array(this.num_agents);
    this.last_input_arrays = new Array(this.num_agents);
    this.average_reward_windows = new Array(this.num_agents);
    this.average_loss_windows = new Array(this.num_agents);
    for (var i = 0; i < this.num_agents ; i ++){
      this.latest_rewards[i] = 0;
      this.last_input_arrays[i] = [];
      this.average_reward_windows[i] = new cnnutil.Window(1000, 10);
      this.average_loss_windows[i] = new cnnutil.Window(1000, 10);
    }
    //console.log(this.average_reward_windows[0]);
    //console.log(this.average_loss_windows[0]);
    this.learning = true;
  }
  Brain.prototype = {
    ...
    getNetInput: function(ai, xt) {
      ...
      for(var k=0;k<this.temporal_window;k++) {
        // state
        w = w.concat(this.state_windows[ai][n-1-k]);
        ...
        action1ofk[this.action_windows[ai][n-1-k]] = 1.0*this.num_states;
        w = w.concat(action1ofk);
      }
      return w;
    },
    // ai: agent id
    forward: function(ai, input_array) {
      // compute forward (behavior) pass given the input neuron signals from body
      this.forward_passes[ai] += 1;
      this.last_input_arrays[ai] = input_array; // back this up

      // create network input
      var action;
      if(this.forward_passes[ai] > this.temporal_window) {
        // we have enough to actually do something reasonable
        var net_input = this.getNetInput(ai, input_array);
        if(this.learning) {
      ...
      this.net_windows[ai].shift();
      this.net_windows[ai].push(net_input);
      this.state_windows[ai].shift();
      this.state_windows[ai].push(input_array);
      this.action_windows[ai].shift();
      this.action_windows[ai].push(action);

      return action;
    },
    backward: function(ai, reward) {
      this.latest_rewards[ai] = reward;
      this.average_reward_windows[ai].add(reward);
      this.reward_windows[ai].shift();
      this.reward_windows[ai].push(reward);
      ...
      if(this.forward_passes[ai] > this.temporal_window + 1) {
        var e = new Experience();
        var n = this.window_size;
        e.state0 = this.net_windows[ai][n-2];
        e.action0 = this.action_windows[ai][n-2];
        e.reward0 = this.reward_windows[ai][n-2];
        e.state1 = this.net_windows[ai][n-1];
      ...
      if(this.experience.length > this.start_learn_threshold) {
        ...
        this.average_loss_windows[ai].add(avcost);
      }
    },
    visSelf: function(elt) {
      ...
      t += 'age: ' + this.age + '<br>';
      t += 'average Q-learning loss: ';
      var avgloss = 0, numloss = 0;
      for (var i = 0; i < this.num_agents; i ++){
        if (this.average_loss_windows[i].v.length !== 0){
          avgloss += this.average_loss_windows[i].get_average();
          numloss ++ ;
        }
      }
      avgloss = avgloss / numloss;
      t += Math.round(avgloss*1e6)/1e6 + '<br />'
      t += 'smooth-ish reward: ';
      var avgreward = 0, numreward = 0;
      for (var i = 0; i < this.num_agents; i ++){
        if (this.average_reward_windows[i].v.length !== 0){
          avgreward += this.average_reward_windows[i].get_average();
          numreward ++ ;
        }
      }
      avgreward = avgreward / numreward;
      t += Math.round(avgreward*1e6)/1e6 + '<br />';
      desc.innerHTML = t;
      brainvis.appendChild(desc);

      //console.log(this.age, avgloss, avgreward);
